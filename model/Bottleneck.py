...
class Bottleneck(nn.Module):
    def __init__(self, ch, internal_ratio=4, kernel_size=3, padding = 0, dilation=1, asymmetric=False, dropout_p = 0, bias=False, relu=False):
        super().__init__()

        if internal_ratio <= 1 or internal_ratio > ch:
            raise RuntimeError("Value out of range. Expected value in the "
                               "interval [1, {0}], got internal_scale={1}."
                               .format(ch, internal_ratio))
        ...
        if asymmetric:
            self.ext_conv2 = nn.Sequential(
                nn.Conv2d(internal_ch, internal_ch, kernel_size=(kernel_size,1), stride=1, padding=(padding,0), dilation=dilation, bias=bias),
                nn.BatchNorm2d(internal_ch),
                activation(),
                nn.Conv2d(internal_ch, internal_ch, kernel_size=(1,kernel_size), stride=1, padding=(0,padding), dilation=dilation, bias=bias),
                nn.BatchNorm2d(internal_ch),
                activation()
            )
        #regular
            ...
            self.ext_conv2 = nn.Sequential(
                nn.Conv2d(internal_ch, internal_ch, kernel_size=kernel_size, stride=1, padding=padding, dilation=dilation, bias=bias),
                nn.BatchNorm2d(internal_ch),
                activation()
            )
...
class DownsamplingBottleneck(nn.Module):
    def __init__(self, input_ch, output_ch, internal_ratio=4, retrun_indices=False, dropout_p = 0, bias=False, relu=False):
        super().__init__()
        ...
        if internal_ratio <= 1 or internal_ratio > input_ch:
            raise RuntimeError("Value out of range. Expected value in the "
                               "interval [1, {0}], got internal_scale={1}. "
                               .format(input_ch, internal_ratio))
        ...
        #Convolution
        self.ext_conv2 = nn.Sequential(
            nn.Conv2d(internal_ch, internal_ch, kernel_size=3, stride=1, padding=1, bias=bias),
            nn.BatchNorm2d(internal_ch),
            activation()
        )
...
class UpsamplingBottleneck(nn.Module):
    def __init__(self, input_ch, output_ch, internal_ratio=4, dropout_p = 0, bias=False, relu=False):
        super().__init__()
        ...
        self.ext_tconv1 = nn.ConvTranspose2d(internal_ch,internal_ch,kernel_size=2,stride=2,bias=bias)
